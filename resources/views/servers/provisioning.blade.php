...
@if($server->provider === \App\Provider::CustomServer && !$isProvisioning)
    <x-splade-modal opened name="provision-script">
        ...
        <p class="mt-4 pr-8 break-all font-mono text-sm relative">
            {{ $server->provisionCommand() }}
            <x-clipboard class="top-0 right-0 w-5 h-5 absolute">{{ $server->provisionCommand() }}</x-clipboard>
        </p>
    </x-splade-modal>
@endif
    ...
    <x-action-section>
        ...
        <x-slot:content>
            <ol role="list" class="space-y-6">
                ...
                <x-dynamic-component
                    :component="$isNew ? 'step.current' : 'step.complete'">
                    {{ __('Create the server at the provider') }}
                </x-dynamic-component>

                <x-dynamic-component
                    :component="$isStarting ? 'step.current' : ($isNew ? 'step.upcoming' : 'step.complete')">
                ...
                @foreach(\App\Server\ProvisionStep::forFreshServer() as $step)
                    ...
                    <x-dynamic-component
                        :component="$completed ? 'step.complete' : ($current ? 'step.current' : 'step.upcoming')">
                        {{ $step->getDescription() }}
                    </x-dynamic-component>
                @endforeach
