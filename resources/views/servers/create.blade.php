...
<x-app-layout>
    <x-action-section>
        ...
        <x-slot:content>
            <x-splade-form :action="route('servers.store')" class="space-y-4" :default="[
                ...
                <div v-if="form.credentials_id && !form.custom_server" class="space-y-4">
                    ...
                    <x-splade-select v-if="form.region" name="type" :label="__('Type')" remote-url="`/servers/provider/${form.credentials_id}/types/${form.region}`" />
                    <x-splade-select v-if="form.region" name="image" :label="__('Image')" remote-url="`/servers/provider/${form.credentials_id}/images/${form.region}`" />
                </div>
