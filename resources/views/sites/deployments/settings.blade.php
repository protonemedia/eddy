<x-site-layout :site="$site" :title="__('Deployment Settings')">
    <x-action-section in-sidebar-layout>
        ...
        <x-slot:content>
            <x-splade-form method="PATCH" :action="route('servers.sites.deployment-settings.update', [$server, $site])" :default="$site" class="space-y-4">
                <x-splade-input
                ...
                @if ($site->zero_downtime_deployment)
                    ...
                    <x-prism-editor
                        ...
                        :help="__('These directories will be shared between the old and new deployment.')"
                    />
                    <x-prism-editor
                        language="plain"
                        name="shared_files"
                        :label="__('Shared Files (one per line)')"
                        :help="__('These files will be shared between the old and new deployment.')"
                    />
                    <x-prism-editor
                        language="plain"
                        name="writeable_directories"
                        :label="__('Writeable Directories (one per line)')"
                        :help="__('These directories will be writeable by the webserver.')"
                    />
                @endif
